    ...
    .then((data) => {
      for (let key in data["multiday_tours"]["armenian"]) {
        document.querySelector(`${key}`).innerHTML =
          data["multiday_tours"]["armenian"][key];
      }
    });
}
function allRussian() {
  fetch("../data/data.json")
    ...
    .then((data) => {
      for (let key in data["multiday_tours"]["russian"]) {
        document.querySelector(`${key}`).innerHTML =
          data["multiday_tours"]["russian"][key];
      }
    });
}
function allEnglish() {
  fetch("../data/data.json")
    ...
    .then((data) => {
      for (let key in data["multiday_tours"]["english"]) {
        document.querySelector(`${key}`).innerHTML =
          data["multiday_tours"]["english"][key];
      }
    });
}
